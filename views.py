...
from django.contrib import messages  # Import messages for displaying alerts to users
from django.shortcuts import render, redirect
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if live_price is None:
                messages.error(request, "Failed to retrieve live price.")
                return redirect('investment')
            ...
            if action == 'buy':
                ...
                process_buy(request.user, ticker, shares, live_price, total_value)

            elif action == 'sell':
                # Process the sell action
                process_sell(request.user, ticker, shares, live_price)

        except ValueError:
            messages.error(request, "Invalid input. Please enter numeric values for shares.")

        return redirect('investment')

    investments = Investment.objects.filter(user=request.user)
    total_investment_value = sum(investment.total_value for investment in investments)

    return render(request, 'investment.html', {
        'investments': investments,
        'total_investment_value': total_investment_value,
        'available_tickers': ['AAPL', 'GOOGL', 'AMZN']  # Example tickers
    })
...
def process_buy(user, ticker, shares, live_price, total_value):
    if live_price > total_value / shares:  # Check if live price is favorable
        ...
        messages.warning(request, "Live price is not favorable for buying.")

def process_sell(user, ticker, shares, live_price):
    existing_investments = Investment.objects.filter(user=user, ticker=ticker)

    if existing_investments.exists():
        investment = existing_investments.first()
        if investment.shares >= shares:  # Ensure there are shares to sell
            total_value = shares * live_price
            investment.shares -= shares  # Decrease the shares
            investment.total_value -= total_value  # Update total value
            investment.save()
            messages.success(request, "Stock sold successfully!")
        else:
            messages.warning(request, "You don't have enough shares to sell.")
    else:
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if action == 'buy':
                ...
                # Mark the investment as sold
                investment.payment_status = True
                investment.shares = 0  # Reset shares or handle as needed
                investment.total_value = 0  # Reset total value or calculate as needed
                investment.save()

            return redirect('investment')  # Redirect back to the investment page
...
def predict(request, ticker_value, number_of_days):
    try:
        ...
        print(f"Error downloading data for {ticker_value}: {e}")
    ...
    try:
        ...
        print(f"Invalid number_of_days format: {e}")
    ...
    try:
        ...
        print(f"Error downloading data for ML: {e}")
        ticker_value = 'AAPL'
    ...
    predicted_price = forecast[-1] if forecast else None  # Get the last predicted price
